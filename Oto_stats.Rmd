...
#load annuli
annuli <- read.delim("Pilot/Aging/Pilot_oto_annuli.txt", sep = "\t", header = T, 
                   stringsAsFactors = F)
...
annu <- annuli%>%
 ...
 mutate(inc = case_when(I %in% "core"~"0",
                         ...
                         TRUE~I))%>%
 ungroup()%>%
 transmute(fishID = substr(Sample, 3,10),
           simsID = gsub("-", "",substr(Sample,0,1)),
           quality = Quality,
           age = Age,
           cohort = Cohort,
           core = Core,
...
#load SIMS data
sims <- read.delim("Otolith_data_2021_clean_with_distance_um.txt", sep = ";", 
...
#load pike data
fishdata <- read.delim("Scales_Oto_data.txt", header = T, 
                       stringsAsFactors = F)
...
pikedata <- annu%>%
  inner_join(sims2, by = c("simsID", "fishID", "year"))
...
for (i in 1:length(IDOto)) {
  png(paste0("Pilot/Test_plots/d18O/", IDOto[i], "_d18O.png"))
...
#write.table(annu, "Pilot/annuli_with_rad.txt", sep = "\t", dec = ".", row.names = F)
```
...
#save table
write.table(pike_d18O, "Pilot/pike_data_d18O_years.txt", sep = "\t", 
            dec = ".", row.names = F)
...
#load trace elements
elements <- read.delim("Otolith_LA-data_2021_all_with_distance.txt", sep = "\t", 
                       header = T, stringsAsFactors = F)
...
elements <- elements%>%filter(sims_id%in%annu$simsID==TRUE)
...
#preliminary save
write.table(pikedata, "Pilot/pike_data_LA-ICPMS_years_with_core.txt", sep = "\t", 
            dec = ".", row.names = F)
```
...
#load d18O data
pike_d18O <- read.delim("Pilot/pike_data_d18O_years.txt", sep = "\t", dec = ".",
                      header = T, stringsAsFactors = F)

#load LA-ICPMS data
pike_elements <- read.delim("Pilot/pike_data_LA-ICPMS_years_with_core.txt", sep = "\t",
...
#All element plots
for (i in 1:length(IDOto)) {
  png(paste0("Pilot/Test_plots/Elements/", IDOto[i], "_Sr.png"), width = 1200, height = 1200)
  par(mfrow=c(6,1))
  #d18O
  plot(pike_d18O[which(pike_d18O$ID == IDOto[i]), "sims_dist",], 
  ...
  plot(pike_elements[which(pike_elements$ID == IDOto[i]), "LA_dist",], 
       pike_elements[which(pike_elements$ID == IDOto[i]), "Sr",], main = "Sr/Ca", 
...
#save table
#remove BH-01741 (vaterite)
pike_elements <- pike_elements%>%filter(ID!="BH-01741")

write.table(pike_elements, "Pilot/pike_data_LA-ICPMS_years.txt", sep = "\t", 
            dec = ".", row.names = F)
```

```{r group data together with main sample}
